...
class MAPDataset(Dataset):
    def __init__(
        ...
        query=None,
        train_mode=False,
        train_formats=[2],  # 默认只用最详细的 format 2
        test_format=2,
    ):
        """
        初始化 Dataset
        ...
        # 1. 构造 Label (Category : Misconception)
        ...
        # 4. 计算当前样本的真实 Label 在候选列表中的索引 (用于 Loss 计算)
        df["cat_id"] = df.apply(
    ...
    def _getitem(self, idx, format_idx):
        data = self.df.iloc[idx]
        fmt_str = DATA_FORMATS[format_idx]

        messages = [
            {"role": "user", "content": fmt_str.format(**data)},
        ]
        ...
        # suffix_ids 用于标记哪部分是候选答案，哪部分是 prompt (-1 表示 prompt)
        ...
        # 4. 拼接
        # last_tokens 记录每个 suffix 的最后一个 token 的位置（通常用于取 logits）
        ...
        # 5. 构建 Label (One-hot 形式，但这里用 float 方便后面计算)
    ...
    def collate_fn(self, samples, block_size=128):
        ...
        处理变长序列的 Batch
        """
        # 如果 samples 是 list of lists (因为 __getitem__ 返回的是 list)，先展平
        _samples = []
        ...
        # 构建 doc_ids 以区分同一个 batch 中的不同样本
        for sample in samples:
        ...
        # Padding (为了适应 block_size 或 GPU 对齐)
        # 可能需要根据实际显存调整 block_size
        if input_ids.size(0) % block_size != 0:
            ...
            suffix_ids = F.pad(suffix_ids, [0, pad_size], value=-2)  # -2 表示 padding
...
class DatasetManager(object):
    @staticmethod
    def load_dataset(cfg, tokenizer, mode="train"):
        """
        根据 ExperimentConfig 创建 MAPDataset 实例
        """
        dataset_config = cfg.dataset_config

        # 1. 确定 Query (数据划分逻辑)
        # config 中有 test_fold, n_folds
        test_fold = cfg.test_fold

        if mode == "train":
            # 训练集: 排除 test_fold
            query = f"fold != {test_fold}"
            is_train = True
        ...
        print(
            f"Loading {mode} dataset with query: '{query}' from {dataset_config.csv_file}"
        )
