...
def parse_args():
    ...
    parser.add_argument("--output-root", type=str, default="artifacts", help="Root directory for outputs")
    parser.add_argument("--load-from", type=str, default=None, help="Path to checkpoint to load")
    parser.add_argument("--eval-only", action="store_true", help="Run evaluation only")
    parser.add_argument("--no-log-file", action="store_true", help="Do not save log to file")
    parser.add_argument("--seed", type=int, default=-1, help="Force specific random seed")
    parser.add_argument("--out", type=str, default=None, help="Path to save evaluation results")
...
def do_test(cfg, model, tokenizer, logger):
    ...
    for batch in prog_bar:
        ...
        for _logits, _label in zip(logits.split(batch["num_candidates"]), batch["label"]):
            # 计算 loss 仅用于进度条显示
    ...
    for curr_epoch in range(max_epochs):
        ...
        for curr_iter, batch in enumerate(epoch_iterator):
            ...
            if total_updates % cfg.log_interval == 0:
                ...
                # 更新进度条描述
                epoch_iterator.set_postfix(loss=f"{loss_val:.4f}", lr=f"{lr:.2e}", mem=f"{max_mem_mb:.0f}M")
        ...
        if (curr_epoch + 1) % cfg.eval_interval == 0:
            ...
            swan_metrics = {f"val/{k}": v for k, v in eval_result.items() if isinstance(v, (int, float))}
...
def main():
    ...
    if not args.eval_only:
        ...
        swanlab.init(
            ...
            mode="disabled" if args.no_log_file else "cloud", # 如果不想上传云端，可改为 "local"
        )
    ...
    load_path = args.load_from if args.load_from else cfg.llm_config.resume_from_checkpoint
